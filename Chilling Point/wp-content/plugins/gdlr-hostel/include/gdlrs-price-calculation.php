<?php
	...
	*/	
	...
	if( !function_exists('gdlr_validate_date_format') ){
		function gdlr_validate_date_format($date){
			...
			if( $date[0] != '*' ){
				if( strlen($date[0]) != 4 || intval($date[0]) < 1900 ){
					return false;
				}
			}
			if( $date[1] != '*' ){
				if( strlen($date[1]) != 2 || intval($date[1]) > 12 ){
					return false;
				}
			}
			if( $date[2] != '*' ){
				if( strlen($date[2]) != 2 || intval($date[2]) > 31 ){
					return false;
				}
			}
		...
		function gdlr_is_ss($selected_date, $ss_price) {
			if( !empty($ss_price['date']) ){
				...
				foreach($date_set as $date){
					// check date format
					$tmp = explode('to', $date);
					if( empty($tmp[0]) || !gdlr_validate_date_format($tmp[0]) ){
						continue;
					}
					if( !empty($tmp[1]) && !gdlr_validate_date_format($tmp[1]) ){
						continue;
					}
